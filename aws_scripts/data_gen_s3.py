...
def save_data_with_metadata(df: pd.DataFrame, output_path: str):
    """Save data with metadata for reproducibility to S3."""
    # Save main data to S3
    df.to_csv(output_path, index=False, storage_options={})
    ...
    s3 = boto3.client('s3')
    bucket = 'medicodeai-ehr-data'
    key = metadata_path.replace('s3://medicodeai-ehr-data/', '')
    s3.put_object(Bucket=bucket, Key=key, Body=json.dumps(metadata, indent=2))
...
def main():
    """Main function to generate synthetic EHR data and save to S3."""
    ...
    # Save to S3 bucket
    output_path = "s3://medicodeai-ehr-data/raw/synthetic_ehr_data.csv"
